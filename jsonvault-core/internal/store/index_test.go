...
func TestStoreIndexes(t *testing.T) {
	...
	// Test Indexed Query
	docs, _, err := db.ListDocuments("testdb", "users", 10, 0, map[string]string{"email": "alice@example.com"})
	if err != nil {
	...
	docs, _, _ = db.ListDocuments("testdb", "users", 10, 0, map[string]string{"email": "alice@example.com"})
	if len(docs) != 0 {
		t.Fatalf("expected 0 docs after update, got %v", docs)
	}

	// New email should return doc
	docs, _, _ = db.ListDocuments("testdb", "users", 10, 0, map[string]string{"email": "alice2@example.com"})
	if len(docs) != 1 || docs[0].ID != doc1.ID {
	...
	docs, _, _ = db.ListDocuments("testdb", "users", 10, 0, map[string]string{"email": "bob@example.com"})
	if len(docs) != 0 {
