...
import type { MockOperation } from "@/lib/mock-dashboard-store";
...
export default function OperationsPage() {
  const { state, selectedDatabase, cancelOperation } = useDashboardMock();
  const [selected, setSelected] = useState<MockOperation | null>(null);
  ...
  const operations = state.operations.filter(
    (operation) => operation.database === selectedDatabase.name,
  );
  ...
  function handleCancel(operationId: string) {
    const result = cancelOperation(operationId);
  ...
  function cancelSelectedOperations() {
    ...
    for (const operation of selectedCancellableOperations) {
      const result = cancelOperation(operation.operation_id);
          ...
          {selectedOperationIds.length > 0 ? (
            <>
              ...
              <button
                ...
                disabled={selectedCancellableOperations.length === 0}
