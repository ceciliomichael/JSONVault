...
export default function FTSPage() {
  ...
  const [fields, setFields] = useState<string[]>(selectedCollection.ftsFields);
  ...
  useEffect(() => {
    setFields(selectedCollection.ftsFields);
    setSelectedFields([]);
    setFieldSearch("");
    setSearchQuery("");
    setNotice("");
  ...
  const results = useMemo(() => {
    ...
    if (!query || selectedCollection.ftsFields.length === 0) return [];
    return selectedCollection.documents.filter((doc) =>
      selectedCollection.ftsFields.some((field) =>
        String(doc.document[field] ?? "")
          .toLowerCase()
          .includes(query),
      ),
    );
  ...
  function saveFields() {
    const result = saveFTSFields(fields);
  ...
  return (
    <WorkspacePage
      ...
      action={
        <Badge
          variant={
            selectedCollection.ftsFields.length > 0 ? "success" : "neutral"
          }
        >
          {selectedCollection.ftsFields.length > 0 ? "Configured" : "Not set"}
        </Badge>
      }
    >
      {notice && (
        <ToastNotice
          message={notice}
          variant={notice.includes("at least") ? "warning" : "success"}
          onClose={() => setNotice("")}
        />
      )}
              ...
              <div className="p-6 flex flex-col gap-6 min-h-0 flex-1">
                <div className="relative shrink-0">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-4 py-3 text-[14px] bg-zinc-50 dark:bg-[#121212] border border-zinc-200 dark:border-white/10 rounded-md focus:outline-none focus:border-zinc-500 focus:ring-1 focus:ring-zinc-500 placeholder:text-zinc-500 text-zinc-900 dark:text-zinc-100 transition-colors shadow-inner"
                  />
                </div>

                <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar pb-16">
                  {selectedCollection.ftsFields.length === 0 ? (
                    <EmptyState
                      icon={FileSearch}
                    ...
                    <div className="h-full flex items-center justify-center p-8">
                      <div className="text-[13px] text-zinc-500 text-center">
                        Type a query to test search against this collection.
                      </div>
                    </div>
                  )}
                </div>
