...
export default function WebhooksPage() {
  ...
  const deliveries = useMemo(
    () =>
      state.deliveries.filter(
        (delivery) => delivery.event.database === selectedDatabase.name,
      ),
    [state.deliveries, selectedDatabase.name],
  );
  const visibleTargets = useMemo(() => {
    const query = targetSearch.trim().toLowerCase();
    if (!query) return selectedCollection.webhooks;
    return selectedCollection.webhooks.filter(
      (target) =>
  ...
  }, [selectedCollection.webhooks, targetSearch]);
  ...
  function handleSaveTarget() {
    setSaveConfirm(false);
    const result = saveWebhookTarget(newUrl, newEvents);
  ...
  function confirmRemoveSelectedTargets() {
    ...
    let lastMessage = "";
    for (const url of targetRemoveUrls) {
      const result = removeWebhookTarget(url);
  ...
  return (
    ...
    >
      {notice && (
        <ToastNotice
          message={notice}
          variant={notice.includes("Use a public") ? "warning" : "success"}
          onClose={() => setNotice("")}
        />
      )}
            ...
            <div className="flex items-center gap-1">
              ...
              {tab === "deliveries" && (
                <button
                  ...
                  onClick={() => setNotice("Delivery records are up to date.")}
                        ...
                        <td className="px-4 py-3 text-right">
                          {delivery.status === "failed" && (
                            <button
                              type="button"
                              onClick={() => {
                                const result = retryDelivery(delivery.sequence);
                                setNotice(result.message);
                              }}
                              className="px-2.5 py-1 rounded-md text-[11px] font-medium border border-zinc-200 dark:border-white/10 text-zinc-500 hover:text-zinc-900 dark:hover:text-zinc-100 hover:bg-zinc-50 dark:hover:bg-zinc-800/50 opacity-0 group-hover:opacity-100 transition-all shadow-sm bg-white dark:bg-[#1a1a1a]"
                            >
                              Retry
                            </button>
                          )}
